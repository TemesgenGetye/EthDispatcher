...
import { Search, Plus, Package, Clock, CheckCircle, AlertCircle, MapPin, User, DollarSign } from 'lucide-react';
import { useOrders } from '../../hooks/useOrders';
import { useDrivers } from '../../hooks/useDrivers';
import CreateOrderModal from './CreateOrderModal';

const AdminOrdersView: React.FC = () => {
  const { orders, loading, error, updateOrder, assignDriver, updateOrderStatus } = useOrders();
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'out_for_delivery': return <Clock className="w-4 h-4 text-blue-600" />;
      case 'assigned': return <User className="w-4 h-4 text-purple-600" />;
      case 'pending': return <Package className="w-4 h-4 text-gray-600" />;
      case 'cancelled': return <AlertCircle className="w-4 h-4 text-gray-500" />;
      case 'returned': return <AlertCircle className="w-4 h-4 text-orange-600" />;
    ...
    <div className="flex-1 bg-gray-50 p-6 overflow-y-auto">
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            <div className="text-2xl font-bold text-blue-600">{statusCounts.outForDelivery}</div>
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            <div className="text-2xl font-bold text-orange-600">{statusCounts.returned}</div>
        ...
        {filteredOrders.map((order) => (
          <div key={order.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center space-x-3">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-semibold text-gray-900">{order.serial_number}</span>
                </div>
                <div className={`w-3 h-3 rounded-full ${getPriorityColor(order.priority)}`} title={`${order.priority} priority`}></div>
              </div>
              <div className="flex items-center space-x-3">
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              {/* Customer & Locations */}
              <div>
                <h4 className="font-semibold text-gray-900 mb-3">{order.customer_name}</h4>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium text-gray-900">{order.items_count} packages</span>
                </div>
                {order.weight_kg && (
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">Weight</span>
                    <span className="text-sm font-medium text-gray-900">{order.weight_kg} kg</span>
                  </div>
                )}
                {order.distance_km && (
                  <div className="flex items-center justify-between">
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-sm text-gray-600 block mb-1">Assigned Driver</label>
                  <select
                    value={order.driver_id || ''}
                    onChange={(e) => e.target.value && handleDriverAssignment(order.id, e.target.value)}
                ...
                {order.estimated_time && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-medium text-gray-900">{order.estimated_time}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      {showCreateModal && (
        <CreateOrderModal
          ...
          onSubmit={async (orderData) => {
            // This will be handled by the CreateOrderModal component
